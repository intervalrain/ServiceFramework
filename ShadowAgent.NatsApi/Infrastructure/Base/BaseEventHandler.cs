    ...
    : MessageTransportBase(broker, bus)
{
    ...
    public async Task HandleInputEvent(byte[] message, string subject)
    {
        ...
        {
            Logger.LogError($"ValidationException in handler: {ex.Message} for {subject}");
            Logger.LogError($"Stack trace: {ex.StackTrace}");
            var msg = "";
            foreach (var err in ex.ValidationErrors)
            {
                Logger.LogError($"Validation Error: {err.MemberNames} - {err.ErrorMessage}");
                msg += $"{err.MemberNames} - {err.ErrorMessage}";
            }

            await HandleValidationException(ex, message, subject);
        }
        // catch (InvalidProtocolBufferException ex)
        // {
        //     Logger.LogError(ex, "The message doesn't obey Protocol rule which contained a tag with an invalid wire type");
        //     await HandleInvalidProtocolBufferException(ex, message, subject);
        // }
        catch (Exception ex)
        {
    ...
    /// <summary>
    /// Handles AbpValidation exceptions. Can be overridden in derived classes.
    /// </summary>
    /// <param name="ex">The validation exception.</param>
    /// <param name="message">The original message bytes.</param>
    /// <param name="subject">The message subject.</param>
    /// <returns>A task that represents the asynchronous operation.</returns>
    protected virtual async Task HandleValidationException(AbpValidationException ex, byte[] message, string subject)
    {
        await HandleGeneralException(ex, message, subject);
    }
