...
using ShadowAgent.Infrastructure.Nats.Attributes;
using ShadowAgent.Infrastructure.Nats.Models;
...
/// </summary>
public abstract class ServiceFramework : MessageTransportBase, IDisposable
{
    ...
    public ILogger<ServiceFramework> Logger { get; }
    
    public ServiceFramework(
        ILogger<ServiceFramework> logger,
    ...
    public override async Task StartAsync(CancellationToken cancellationToken)
{
    ...
    /// <summary>
    /// Lists all subclasses of the ServiceFramework class.
    /// </summary>
    /// <returns>A list of Type objects representing the subclasses of ServiceFramework.</returns>
    public static List<Type> ListSubclasses()
    {
        var subclasses = new List<Type>();
        var baseType = typeof(ServiceFramework);
    ...
    public async Task ConnectAsync(CancellationToken cancellationToken = default)
    {
        ...
        try
        {   // all the microservices with service framework should have the same connection options and credentials.
            // and connects to the message bus.
            NatsOpts opts = NatsOpts.Default with
            {
                Url = Configuration.ServiceConfig.MsgBusUrl,
                AuthOpts = NatsAuthOpts.Default with { CredsFile = Configuration.ServiceConfig.MsgBusCredFile }
            };
    ...
    public async Task AddEndpointAsync<T>(string endpointName, Func<ServiceMsgContex<T>, T, ValueTask> handler, string? customSubject = null, CancellationToken cancellationToken = default)
    {
        if (_svcServer == null)
        {
            throw new Exception("Nats connection is not established.");
        ...
        try
        {
            await _svcServer.AddEndpointAsync<T>(
                ...
                handler: async (msg) =>
                {
                    var svcMsgCtx = new ServiceMsgContex<T>
                    {
                        ServiceMsg = msg
                    };
                    try
                    ...
                    catch (Exception ex)
                    {
                        ...
                        string protoVer = string.Empty;

                        TryParseSubject(svcMsgCtx.Subject, out protoVer, out groupId, out deviceId);
    ...
    /// <returns>A task representing the asynchronous operation.</returns>
    public async Task ReplyAsync<T, TR>(ServiceMsgContex<T> svcMsgCtx, TR msg)
    {
    ...
    protected async Task RegisterEndpointsAsync(CancellationToken cancellationToken = default)
    {
        ...
        foreach (var method in methods)
        {
            ...
            try
            {
                ...
                if (!ctxParamType.IsGenericType || ctxParamType.GetGenericTypeDefinition() != typeof(ServiceMsgContex<>))
                {
                ...
                var handlerType = typeof(Func<,,>).MakeGenericType(
                    typeof(ServiceMsgContex<>).MakeGenericType(msgType),
                    msgType,
                    typeof(ValueTask));
                var handler = Delegate.CreateDelegate(handlerType, this, method);

                // 調用 AddEndpointAsync
                var addEndpointMethod = typeof(ServiceFramework).GetMethod(nameof(AddEndpointAsync))
    ...
    public T ParseApiRequest<T>(ServiceMsgContex<string> svcMsgCtx, string message, string command) where T : class, IServiceBasicDto
    {
        // Parse subject
